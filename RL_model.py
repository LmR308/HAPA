    ...
    def train_RL(self, train_loader, test_loader):
        ...
        while num_visits < tl_visits:
            ...
            for epoch in range(self.opt.model_epochs):
                ...
                loss_matrix_tensor = torch.stack([torch.stack(row) for row in loss_matrix]).cuda()# loss_martix.shape=(singleCapacity, num_classes)
                result_matrix = torch.mm(loss_matrix_tensor.T.cuda(), torch.tensor(action_martix, dtype=torch.float32, requires_grad=True).cuda())# result_matrix.shape=(num_classes, num_classes)
