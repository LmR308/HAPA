...
class OutputLayer(nn.Module):
    def __init__(
        ...
        if representation_size1:
            ...
            # modules.append(nn.Linear(embedding_dim, 1024))
            # modules.append(nn.Sigmoid())
            # # modules.append(nn.ReLU())
            # # modules.append(nn.Tanh())
...
class CNN(nn.Module):
    def __init__(self, embedding_dim, opt, mlp_ratio, qkv_bias=True,use_revised_ffn=False,
        ...
        self.opt = opt
        if self.opt.CNN_USE_TRANSFORM == str('true'):
...
class Submodel(nn.Module):
    def __init__(self,input_dim, output_dim, embedding_dim, opt, mlp_ratio, qkv_bias=True,use_revised_ffn=False,
        dropout_rate=0.7,attn_dropout_rate=0.7):
        super(Submodel, self).__init__()
        self.n1 = nn.Linear(input_dim, 200).cuda()
        self.n2 = nn.Linear(200, output_dim).cuda()
        self.opt = opt
        if self.opt.Submodel_USE_TRANSFORM == str('true'):
            self.transformer = Transformer(
                dim=embedding_dim,
                depth=opt.num_layers,
                heads=opt.num_heads,
    ...
    def forward(self, x):
        ...
        x = torch.sigmoid(self.n1(x))
        x = torch.sigmoid(self.n2(x))
        return x
...
class AesModel(nn.Module):
    def __init__(
        ...
        self.CNN = CNN(embedding_dim, opt, mlp_ratio)
        
        self.submodels = nn.ModuleList([Submodel(embedding_dim, 1, embedding_dim, opt, mlp_ratio) for _ in range(opt.num_classes)])
    ...
    def forward(self, x):
        ...
        output = torch.cat([model(cnn_output) for model in self.submodels], dim=1)
